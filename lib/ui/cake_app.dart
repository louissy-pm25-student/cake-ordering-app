...
class CakeApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MaterialApp(
    ...
    home: ListenableBuilder(
      ...
      builder: (context, _) {
        final vm = viewModel;
        if (vm.isAdmin) return AdminScreen(vm.admin, onLogout: vm.exitAdmin);
        return PopScope(
          ...
          child: Scaffold(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Stack(
                          children: [
                            CakeNavHost(vm),
                            if (CakeNavigation.showsCart(vm.destination) &&
                                vm.cart.isNotEmpty)
                              Positioned(
                                left: 16,
                                right: 16,
                                ...
                                child: PrimaryButton(
                                  ...
                                  () => vm.navigate(CakeDestination.checkout),
                                ),
                              ),
                          ],
                        ),
            ...
            bottomNavigationBar:
                (vm.destination == CakeDestination.login ||
                    vm.destination == CakeDestination.register)
                ? null
                : Container(
                    ...
                    child: SafeArea(
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          children: CakeNavigation.bottomDestinations.map((
                            page,
                          ) {
                            final selected = vm.destination == page;
                            final custom = page == CakeDestination.customize;
                            return Expanded(
                              child: Semantics(
                                selected: selected,
                                button: true,
